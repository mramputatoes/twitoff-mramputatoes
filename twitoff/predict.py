""" Prediction of user based on tweet vectors """
import numpy as np
...
def predict_user(user0_name, user1_name, hypo_tweet_text):
    ...
    user0_vects = np.array([tweet.vect for tweet in user0.tweet])
    user1_vects = np.array([tweet.vect for tweet in user1.tweet])
    ...
    labels = np.concatenate(
        [np.zeros(len(user0.tweet)), np.ones(len(user1.tweet))])
